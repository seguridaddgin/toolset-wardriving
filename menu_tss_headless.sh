#!/bin/bash
# kismet_menu.sh - Administrador interactivo de Kismet con dialog/ncurses
...
status_kismet() {
    if [ -f "$PIDFILE" ] && ps -p $(cat "$PIDFILE") > /dev/null; then
        STATUS="Kismet está corriendo en $KISMET_IFACE con PID $(cat $PIDFILE)."
    else
...
while true; do
    ...
    CHOICE=$(dialog --clear --stdout \
        --title " ToolSet Wardriving Scout | $STATUS_KISMET_MSG | $STATUS_GPSD_MSG " \
        --menu "Seleccione una acción:" 16 70 6 \
        1 "Iniciar Kismet" \
        2 "Detener Kismet" \
        3 "Reiniciar Kismet" \
        4 "Estado de Kismet" \
        5 "Ver archivo de captura" \
	6 "Ver estado gpsd" \
